...
RUN chmod 777 backend/runtime && chmod 777 api/runtime && chmod 777 wechat/runtime && chmod a+x init && ./init --env=Production && randKey=`openssl rand -hex 16` && sed -i "s/'cookieValidationKey' => ''/'cookieValidationKey' => '${randKey}'/" api/config/main-local.php && sed -i "s/'cookieValidationKey' => ''/'cookieValidationKey' => '${randKey}'/" wechat/config/main-local.php && sed -i "s/'cookieValidationKey' => ''/'cookieValidationKey' => '${randKey}'/" backend/config/main-local.php && sed -i "s/^fastcgi_param PHP_ADMIN_VALUE/#&/" /usr/local/nginx/conf/fastcgi.conf
